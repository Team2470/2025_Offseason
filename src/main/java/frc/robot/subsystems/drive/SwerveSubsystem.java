...
public class SwerveSubsystem extends SubsystemBase {
    ...
    private final Timer pathplannerTimer = new Timer();
        ...
        switch (systemState) {
            ...
            case DRIVE_TO_POINT:
                ...
                if (DriverStation.isAutonomous()) {
                    velocityOutput = Math.min(Math.abs(autoDriveToPointController.calculate(linearDistance, 0)) + frictionConstant, maxVelocityOutputForDriveToPoint);
                } else {
                    velocityOutput = Math.min(Math.abs(teleopDriveToPointController.calculate(linearDistance, 0)) + frictionConstant,maxVelocityOutputForDriveToPoint);
                }
